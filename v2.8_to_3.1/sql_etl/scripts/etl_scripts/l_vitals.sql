...
GRANT ALL ON ALL SEQUENCES IN SCHEMA SITE_pcornet TO pcor_et_user;
...
create temporary table ob_tobacco as ( select distinct observation_id, visit_occurrence_id, observation_date, observation_datetime,coalesce(m1.target_concept,'OT') as tobacco 
	from SITE_pedsnet.observation o1 left join SITE_pcornet.pedsnet_pcornet_valueset_map m1 on cast(o1.value_as_concept_id as text) = m1.source_concept_id
...
create temporary table ob_smoking as (select distinct observation_id, visit_occurrence_id, observation_date, observation_datetime, coalesce(m3.target_concept,'OT') as smoking 
	from SITE_pedsnet.observation o1 left join SITE_pcornet.pedsnet_pcornet_valueset_map m3 on cast(o1.value_as_concept_id as text)= m3.source_concept_id
...
FROM 
vital_extract
left join SITE_pcornet.pedsnet_pcornet_valueset_map m on cast(measurement_concept_id_sys as text) = m.source_concept_id 
