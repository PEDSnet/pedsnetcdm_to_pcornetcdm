...
from sqlalchemy.sql.expression import case, cast, exists, and_, bindparam
                      ...
                      and_(race_value_map.source_concept_class == 'Race',
                           case([(and_(Person.race_concept_id == None,
                                ...
                                else_=cast(Person.race_concept_id,
                                           String(200)) == race_value_map.source_concept_id))) \
            .filter(exists().where(Person.person_id == PersonVisit.person_id)).all()
