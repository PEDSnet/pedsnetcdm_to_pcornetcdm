...
where
	...
	and EXTRACT(YEAR FROM drug_exposure_start_date) >= 2001
        and (de.drug_source_value is null or de.drug_source_value not ilike any (array['%breastmilk%','%kit%','%item%','%formula%', '%tpn%','%custom%','%UNDILUTED DILUENT%','%KCAL/OZ%']));
commit;
...
where rxnorm_cui is null and lower(drug_source_value) ilike any(array['%human milk%','%tpn%','%similac%','%fat emulsion%','%UNDILUTED DILUENT%','%KCAL/OZ%']))
delete from SITE_pcornet.prescribing
