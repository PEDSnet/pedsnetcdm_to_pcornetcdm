...
elect distinct
                ...
                coalesce(px_cd_1.concept_code, px_cd_2.concept_code, split_part(procedure_source_value,'|',2),null)::varchar(11) as px,
    coalesce(px_typ.target_concept,px_typ2.target_concept,'OT')as px_type,
                ...
                'SITE' as site
from stanford_pedsnet.procedure_occurrence po
                join stanford_pcornet.encounter enc on cast(po.visit_occurrence_id as text)=enc.encounterid
    ...
    left join pcornet_maps.pedsnet_pcornet_valueset_map px_typ on px_typ.source_concept_id = px_cd_1.vocabulary_id
                left join pcornet_maps.pedsnet_pcornet_valueset_map px_typ2 on px_typ2.source_concept_id = px_cd_2.vocabulary_id and px_typ2.source_concept_class = 'Procedure Code Type'
                ...
                where  person_id IN (select person_id from stanford_pcornet.person_visit_start2001)
                       and EXTRACT(YEAR FROM procedure_date) >= 2001
                       and visit_occurrence_id is not null
                       and visit_occurrence_id not in (select visit_occurrence_id from stanford_pedsnet.visit_occurrence where
