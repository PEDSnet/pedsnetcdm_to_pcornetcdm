...
def connect():
    ...
    try:
        ...
        cur.execute("""select exists(select 1 from information_schema.schemata where schema_name = \'"""+ schema_path['schema']+"""\');""")
        schema_exist = cur.fetchall()[0]
        print(schema_exist)

        if "True" not in schema_exist:
            cur.execute(query.create_schema(schema_path['schema']))
            print '% schema created' % schema_path['schema']

        # set the search pat to the schema
        cur.execute("""SET search_path TO """ + schema_path['schema'] + """;""")
        ...
        # create table
        try:
            cur.execute(query.create_table(schema_path['schema']))
            print(conn.notices)
            conn.commit()
            ...
            if os.path.isfile('concept_map/concept_map.csv'):
                ...
                cur.copy_from(f, schema_path['schema'] + ".pedsnet_pcornet_valueset_map", columns=(
