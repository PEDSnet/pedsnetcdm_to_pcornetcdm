...
.PHONY: all
all: dcc_3dot1 dcc_3dot1_start2001 site_3dot1 site_3dot1_start2001

%:
	@if [ "${DB}" == "none" ]; then echo "Invoke as: make -f create_pcornet_3.1_tables.Makefile DB=thedb VER=theversion"; false; fi
	@if [ "${VER}" == "none" ]; then echo "Invoke as: make -f create_pcornet_3.1_tables.Makefile DB=thedb VER=theversion"; false; fi
	mkdir -p ${LOGDIR}
