...
begin;
delete from SITE_pcornet.immunization imz
where NOT EXISTS (select 1 from SITE_pedsnet.procedure_occurrence pco
                                        WHERE pco.procedure_occurrence_id = imz.proceduresid::int
                                                and pco.person_id IN (select person_id from SITE_pcornet.person_visit_start2001)
                                                and (pco.visit_occurrence_id is NULL or pco.visit_occurrence_id IN (select visit_id from SITE_pcornet.person_visit_start2001)));
commit;
