...
insert into chop_start2001_pcornet.encounter (
...
WITH  o1 as (select distinct person_id,visit_occurrence_id,value_as_concept_id, observation_source_value from chop_pedsnet.observation where observation_concept_id = 44813951)
     ,o2 as (select distinct person_id,visit_occurrence_id, value_as_string
		from chop_pedsnet.observation
     ...
     ,o3 as (select distinct person_id,visit_occurrence_id, value_as_concept_id,observation_source_value from chop_pedsnet.observation where observation_concept_id = 4137274)
     ,o4 as (select distinct value_as_concept_id, visit_occurrence_id, person_id,observation_source_value from chop_pedsnet.observation where observation_concept_id = 4145666)
select distinct 
	...
	v.site as site
from 
	chop_pedsnet.visit_occurrence v
	left join chop_pedsnet.care_site c on v.care_site_id = c.care_site_id
	left join chop_pedsnet.location l on c.location_id = l.location_id
	...
	left join chop_start2001_pcornet.cz_omop_pcornet_concept_map m1 on case when v.visit_concept_id is null AND m1.source_concept_id is null then true else 	cast(v.visit_concept_id as text)= m1.source_concept_id end and m1.source_concept_class='Encounter type'
	left join chop_start2001_pcornet.cz_omop_pcornet_concept_map m2 on case when o1.value_as_concept_id is null AND m2.value_as_concept_id is null then true else o1.value_as_concept_id = m2.value_as_concept_id end and m2.source_concept_class='Discharge disposition'
	left join chop_start2001_pcornet.cz_omop_pcornet_concept_map m3 on case when o3.value_as_concept_id is null AND m3.value_as_concept_id is null then true else o3.value_as_concept_id = m3.value_as_concept_id end and m3.source_concept_class='Discharge status'
	left join chop_start2001_pcornet.cz_omop_pcornet_concept_map m4 on case when o4.value_as_concept_id is null AND m4.value_as_concept_id is null then true else o4.value_as_concept_id = m4.value_as_concept_id end and m4.source_concept_class='Admitting source'
where
	v.visit_occurrence_id IN (select visit_id from chop_start2001_pcornet.person_visit_start2001)
group by
    v.person_id,
