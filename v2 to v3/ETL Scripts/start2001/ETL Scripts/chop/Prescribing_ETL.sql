...
insert into chop_start2001_pcornet.prescribing (prescribingid,
            patid, encounterid, 
...
select distinct
	...
	de.site as site
from
	chop_pedsnet.drug_exposure de
	join chop_start2001_pcornet.demographic d on d.patid = cast(de.person_id as text)
	join chop_start2001_pcornet.encounter e on cast(de.visit_occurrence_id as text) = e.encounterid
	left join chop_start2001_pcornet.cz_omop_pcornet_concept_map m1 on case when de.drug_type_concept_id is null 
...
where
	...
	and de.person_id IN (select person_id from chop_start2001_pcornet.person_visit_start2001);
