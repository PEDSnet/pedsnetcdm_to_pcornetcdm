...
-- condition_occurrence --> Diagnosis
delete from chop_start2001_pcornet.diagnosis

insert into chop_start2001_pcornet.diagnosis(
...
+				else  trim(condition_source_value)  end  end end  
	...
	co.site as site
from
	chop_pedsnet.condition_occurrence co
	join vocabulary.concept c2 on co.condition_concept_id = c2.concept_id
	join chop_start2001_pcornet.encounter enc on cast(co.visit_occurrence_id as text)=enc.encounterid
	left join chop_start2001_pcornet.cz_omop_pcornet_concept_map m1 on m1.source_concept_class='dx_source' and cast(co.condition_type_concept_id as text) = m1.source_concept_id
	left join chop_start2001_pcornet.cz_omop_pcornet_concept_map m2 on  cast(co.condition_type_concept_id as text) = m2.source_concept_id  and m2.source_concept_class='pdx'
...
where 
	...
	co.visit_occurrence_id IN (select visit_id from chop_start2001_pcornet.person_visit_start2001)
