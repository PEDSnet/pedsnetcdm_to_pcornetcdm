...
from SITE_pedsnet.procedure_occurrence po
	...
	where  person_id IN (select person_id from SITE_pcornet.person_visit_start2001)
	       ...
	       and po.procedure_concept_id not in (select concept_id from vocabulary.concept where concept_class_id='CPT4 Modifier')
	       and po.procedure_source_concept_id not in (select concept_id from vocabulary.concept where concept_class_id='CPT4 Modifier');
