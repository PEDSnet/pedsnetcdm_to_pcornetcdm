...
from 
	...
	join SITE_3dot1_pcornet.pedsnet_pcornet_valueset_map m1 
		on cast(visit_concept_id as text)= m1.source_concept_id and m1.source_concept_class='Encounter type'
	left join SITE_3dot1_pcornet.pedsnet_pcornet_valueset_map m2 on case when value_as_concept_id_ddisp is null AND m2.value_as_concept_id is null then true else 
				cast(value_as_concept_id_ddisp as text) = m2.value_as_concept_id end and m2.source_concept_class='Discharge disposition'
	left join SITE_3dot1_pcornet.pedsnet_pcornet_valueset_map m4a on admitting_source_concept_id = m4a.source_concept_id::integer 
			and m4a.source_concept_class='Admitting source'            
	left join SITE_3dot1_pcornet.pedsnet_pcornet_valueset_map m3a on cast(discharge_to_concept_id as text) = m3a.source_concept_id 
...
--- loading 
insert into SITE_3dot1_pcornet.encounter (
