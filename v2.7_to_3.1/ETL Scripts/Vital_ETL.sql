...
create temporary table ms_ht as (select distinct measurement_id, visit_occurrence_id, measurement_date, measurement_datetime, value_as_number  from dcc_pedsnet.measurement where measurement_concept_id = '3023540'); 

create temporary table ms_wt as (select distinct measurement_id,visit_occurrence_id, measurement_date, measurement_datetime, value_as_number  from dcc_pedsnet.measurement where measurement_concept_id = '3013762');

create temporary table ms_bmi as (select distinct measurement_id,visit_occurrence_id, measurement_date, measurement_datetime, value_as_number  from dcc_pedsnet.measurement where measurement_concept_id = '3038553');
...
from vital_extract 
left join dcc_3dot1_pcornet.pedsnet_pcornet_valueset_map m on cast(ms_sys.measurement_concept_id_sys as text) = m.source_concept_id AND m.source_concept_class='BP Position'
