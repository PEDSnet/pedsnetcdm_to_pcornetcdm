...
where
	...
	and de.drug_source_value not ilike any (array['%UNDILUTED DILUENT%','%KCAL/OZ%','%breastmilk%','%kit%','%item%','%formula%', '%tpn%','%custom%','%fat emulsion%','%parenteral nutrition%','%ZZBREAST MILK%','%FAT EMULSION%']); 
commit;
...
where medadmin_code is null and lower(drug_source_value) ilike any(array['%UNDILUTED DILUENT%','%KCAL/OZ%','%human milk%','%tpn%','%similac%','%fat emulsion%','%parenteral nutrition%','%ZZBREAST MILK%','%FAT EMULSION%']))
delete from SITE_pcornet.med_admin
