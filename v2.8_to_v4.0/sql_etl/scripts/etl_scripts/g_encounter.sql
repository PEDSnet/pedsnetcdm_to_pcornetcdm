...
create  table 
SITE_4dot0_pcornet.dis_disposition as (select distinct person_id,visit_occurrence_id,min(value_as_concept_id) as value_as_concept_id 
...
create  table 
SITE_4dot0_pcornet.drg_value as (select distinct person_id,visit_occurrence_id, min(value_as_string) as value_as_string
		from SITE_pedsnet.observation
...
create  table 
SITE_4dot0_pcornet.primary_visit_payer as 
 with min_payer_data as 
	( select min(visit_payer_id) as visit_payer_id, visit_occurrence_id from
	SITE_pedsnet.visit_payer where 
	visit_occurrence_id in (select visit_id from SITE_4dot0_pcornet.person_visit_start2001) 
	group by visit_occurrence_id
	)
	select vp.visit_occurrence_id, vp.visit_payer_id, plan_name, plan_type, plan_class
...
as 
	...
	visit_payer_id, plan_name, plan_class, plan_type, 
	v.site
from 
	...
	left join SITE_4dot0_pcornet.dis_disposition on v.visit_occurrence_id = dis_disposition.visit_occurrence_id 
...
as 
 select 
	...
	plan_class||'-'||plan_type as raw_payer_type_primary, 
	plan_name as raw_payer_name_primary, 
	visit_payer_id as raw_payer_id_primary,
...
--- loading 
insert into SITE_4dot0_pcornet.encounter (
            ...
            discharge_status, drg, drg_type, admitting_source, payer_type_primary, raw_enc_type, 
 ...
 select 
    ...
    drg, 	drg_type, admitting_source,primary_payer_type, 
	raw_enc_type,
